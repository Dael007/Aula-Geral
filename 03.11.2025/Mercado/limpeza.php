...
if ($q !== '') {
    $stmt = $db->prepare("SELECT id, nome, preco, unidade, estoque FROM limpeza WHERE nome LIKE ? ORDER BY nome");
    $stmt->bind_param('s', $like);
    $stmt->execute();
    $res = $stmt->get_result();
} else {
    $res = $db->query("SELECT id, nome, preco, unidade, estoque FROM limpeza ORDER BY nome");
}
...
<body>
  ...
  <main class="container">
    <input id="busca" type="search" placeholder="Buscar produtos de limpeza..." value="<?php echo htmlspecialchars($q, ENT_QUOTES, 'UTF-8'); ?>" style="max-width:420px;width:100%;padding:10px 12px;border:1px solid #ddd;border-radius:8px;">
  ...
  <script>
    ...
    input.addEventListener('input', () => {
      ...
      timer = setTimeout(async () => {
        ...
        const url = `limpeza.php?partial=1&q=${q}`;
        const res = await fetch(url, { headers: { 'X-Requested-With': 'fetch' } });
        const html = await res.text();
        tbody.innerHTML = html;
        history.replaceState(null, '', `limpeza.php?q=${q ? q : ''}`);
      }, 300);
    });
  </script>
</body>
